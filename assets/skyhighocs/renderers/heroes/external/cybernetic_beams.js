...
function bindBeam(renderer, propertyName, anchor, color, entries) {
  ...
  var beamRenderer = renderer.createResource("BEAM_RENDERER", "skyhighocs:cybernetic_cannons");

  if (overrides.hasOwnProperty(color)) {
    beamRenderer = renderer.createResource("BEAM_RENDERER", "skyhighocs:" + overrides[color] + "_cannons");
  };
...
function initSatelliteBeams(renderer, model, color) {
  ...
  if (overrides.hasOwnProperty(color)) {
    transmitBeamRenderer = renderer.createResource("BEAM_RENDERER", "skyhighocs:" + overrides[color] + "_transmit");
  };
  ...
  if (overrides.hasOwnProperty(color)) {
    receiveBeamRenderer = renderer.createResource("BEAM_RENDERER", "skyhighocs:" + overrides[color] + "_receive");
  } else {
    receiveBeamRenderer = renderer.createResource("BEAM_RENDERER", "skyhighocs:cybernetic_receive");
  };
  ...
  return {
    render: function (entity, isFirstPersonArm) {
      ...
      if (!isFirstPersonArm) {
        if (entity.world().isUnobstructed(entity.eyePos().add(0, 1, 0), vector) && entity.getData("skyhighocs:dyn/satellite_deployed")) {
          if (transmitTimer > 0) {
            transmitBeam.render();
          };
          if (receiveTimer > 0) {
    ...
    render: function (entity, isFirstPersonArm) {
      if (!isFirstPersonArm) {
        ...
        entities.forEach(otherEntity => {
          ...
          if (factor > 0 && entity.getData("skyhighocs:dyn/antenna_deploy_timer") == 1) {
            transmitBeam.render();
            receiveBeam.render();
          };
          /* if (hasCyberneticBody(otherEntity) && checkFrequency(entity, otherEntity)) {
            if (entity.world().isUnobstructed(entity.eyePos().add(0, 1, 0), otherEntity.eyePos().add(0, 1, 0)) && entity.getData("skyhighocs:dyn/antenna_deployed") && otherEntity.getData("skyhighocs:dyn/antenna_deployed")) {
            };
          }; */
        });
      };
