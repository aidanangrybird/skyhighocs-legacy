...
function initModule(system) {
  ...
  function battleCardMessage(entity) {
    ...
    if (entity.getData("skyhighocs:dyn/selected_battle_card") == 1) {
      system.systemMessage(entity, "<n>Selected <nh>Electrostatic Dome<n> battle card!");
    };
    if (entity.getData("skyhighocs:dyn/selected_battle_card") == 2) {
      system.systemMessage(entity, "<n>Selected <nh>Taser<n> battle card!");
    };
    if (entity.getData("skyhighocs:dyn/selected_battle_card") == 3) {
      system.systemMessage(entity, "<n>Selected <nh>Shock<n> battle card!");
    };
    if (entity.getData("skyhighocs:dyn/selected_battle_card") == 4) {
      system.systemMessage(entity, "<n>Selected <nh>Arc<n> battle card!");
    };
    if (entity.getData("skyhighocs:dyn/selected_battle_card") == 5) {
      system.systemMessage(entity, "<n>Selected <nh>Over Charge<n> battle card!");
    };
  };
  return {
    name: "ecolyte",
    ...
    powers: [
      ...
      "skyhighocs:spark_buster"
    ],
    keyBinds: function (hero) {
      hero.addKeyBind("GRAVITY_MANIPULATION", "Battle Card Predation", 2);
      hero.addKeyBind("AIM", "Aim Spark Buster", 4);
      hero.addKeyBindFunc("BATTLE_CARD_RESET", (player, manager) => resetBattleCard(player, manager), "Return To Spark Buster", 5);
      hero.addKeyBindFunc("PULSE_OUT", (player, manager) => {
        ...
        manager.setData(player, "skyhighocs:dyn/ecolyte_timer", 0);
        manager.setData(player, "skyhighocs:dyn/ecolyte", false);
      ...
      hero.addKeyBindFunc("PULSE_IN", (player, manager) => {
        if (player.getUUID() == system.getCompatibleUUID(player)) {
          system.shoutMessage(player, "<Orion Stelar> EM Wave Change! \u00A73Orion Stelar\u00A7r, On-Air!", 16);
          manager.setData(player, "skyhighocs:dyn/battle_card", 0);
          manager.setData(player, "skyhighocs:dyn/selected_battle_card", 0);
          manager.setData(player, "skyhighocs:dyn/body_temperature", 0.0);
          manager.setData(player, "skyhighocs:dyn/predation_timer", 0);
          ...
          system.shoutMessage(player, "<\u00A73Ecolyte\u00A7r> Who are you?", 16);
        };
        return true;
      }, "Pulse In", 5);
      hero.addKeyBind("WAVE_CHANGE", "EM Wave Change!", 5);
      hero.addKeyBind("ECOLYTE_TOGGLE", "Toggle Spark Buster", 5);
      hero.addKeyBindFunc("COLD_TEMPERATURE", (player, manager) => {
        if (player.getUUID() == system.getCompatibleUUID(player)) {
          system.shoutMessage(player, "\u00A7r<\u00A73Ecolyte\u00A7r> You are too cold for us to pulse in.", 16);
        } else {
          system.shoutMessage(player, "<\u00A73Ecolyte\u00A7r> Who are you?", 16);
        };
        return true;
      ...
      hero.addKeyBindFunc("HOT_TEMPERATURE", (player, manager) => {
        if (player.getUUID() == system.getCompatibleUUID(player)) {
          system.shoutMessage(player, "\u00A7r<\u00A73Ecolyte\u00A7r> You are too hot for us to pulse in.", 16);
        } else {
          system.shoutMessage(player, "<\u00A73Ecolyte\u00A7r> Who are you?", 16);
        };
        return true;
    ...
    isKeyBindEnabled: function (entity, keyBind) {
      ...
      if (keyBind == "GRAVITY_MANIPULATION") {
        result = (entity.getData("skyhighocs:dyn/battle_card") > 0 || entity.getData("skyhighocs:dyn/ecolyte_timer") < 1) && entity.getData("skyhighocs:dyn/wave_changing_timer") == 1;
      };
      ...
      if (keyBind == "AIM") {
        result = ((entity.getHeldItem().isEmpty() && entity.getData("skyhighocs:dyn/battle_card") == 0 && entity.getData("skyhighocs:dyn/ecolyte_timer") < 1) || entity.getHeldItem().name() == "fiskheroes:chronos_rifle") && entity.getData("skyhighocs:dyn/wave_changing_timer") == 1;
      };
