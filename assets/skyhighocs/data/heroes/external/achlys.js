...
function initModule(system) {
  ...
  function battleCardMessage(entity) {
    ...
    if (entity.getData("skyhighocs:dyn/selected_battle_card") == 1) {
      system.systemMessage(entity, "<n>Selected <nh>Void's Veil<n> battle card!");
    };
    if (entity.getData("skyhighocs:dyn/selected_battle_card") == 2) {
      system.systemMessage(entity, "<n>Selected <nh>Soul Splitter<n> battle card!");
    };
    if (entity.getData("skyhighocs:dyn/selected_battle_card") == 3) {
      system.systemMessage(entity, "<n>Selected <nh>Abyssal Shroud<n> battle card!");
    };
    if (entity.getData("skyhighocs:dyn/selected_battle_card") == 4) {
      system.systemMessage(entity, "<n>Selected <nh>Unleashed Darkness<n> battle card!");
    };
    if (entity.getData("skyhighocs:dyn/selected_battle_card") == 5) {
      system.systemMessage(entity, "<n>Selected <nh>Shadow Blast<n> battle card!");
    };
  };
  return {
    name: "achlys",
    type: 8,
    emBeing: "Achlys",
    compatibleHumanUUID: "e51532a1-19fc-4d4f-9da0-f952c4645891",
    powers: [
      "skyhighocs:achlys",
    ...
    keyBinds: function (hero) {
      ...
      hero.addKeyBindFunc("PULSE_IN", (player, manager) => {
        if (player.getUUID() == system.getCompatibleUUID(player)) {
          system.shoutMessage(player, "<Damien Stelar> EM Wave Change! \u00A75Damien Stelar\u00A7r, On-Air!", 16);
          manager.setData(player, "skyhighocs:dyn/battle_card", 0);
          manager.setData(player, "skyhighocs:dyn/selected_battle_card", 0);
          manager.setData(player, "skyhighocs:dyn/body_temperature", 0.0);
          manager.setData(player, "skyhighocs:dyn/predation_timer", 0);
          ...
          system.shoutMessage(player, "<\u00A75Achlys\u00A7r> Who are you?", 16);
        };
        return true;
      }, "Pulse In", 5);
      ...
      hero.addKeyBindFunc("COLD_TEMPERATURE", (player, manager) => {
        if (player.getUUID() == system.getCompatibleUUID(player)) {
          system.shoutMessage(player, "\u00A7r<\u00A75Achlys\u00A7r> You are too cold for us to pulse in.", 16);
        } else {
          system.shoutMessage(player, "<\u00A75Achlys\u00A7r> Who are you?", 16);
        };
        return true;
      ...
      hero.addKeyBindFunc("HOT_TEMPERATURE", (player, manager) => {
        if (player.getUUID() == system.getCompatibleUUID(player)) {
          system.shoutMessage(player, "\u00A7r<\u00A75Achlys\u00A7r> You are too hot for us to pulse in.", 16);
        } else {
          system.shoutMessage(player, "<\u00A75Achlys\u00A7r> Who are you?", 16);
        };
        return true;
    ...
    isKeyBindEnabled: function (entity, keyBind) {
      ...
      if (keyBind == "AIM") {
        result = ((entity.getHeldItem().isEmpty() && entity.getData("skyhighocs:dyn/battle_card") == 0 && entity.getData("skyhighocs:dyn/achlys_timer") < 1) || entity.getHeldItem().name() == "fiskheroes:chronos_rifle") && entity.getData("skyhighocs:dyn/wave_changing_timer") == 1;
      };
